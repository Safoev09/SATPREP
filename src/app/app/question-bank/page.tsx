        ...
        return (
          <div key={section} className="mb-10">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-display text-2xl font-semibold text-coffee-900 dark:text-cream-50">
                  {sectionTitle}
                </h2>
                <Link
                  href={drillHref}
            ...
            <div className="space-y-5">
              {Object.entries(sectionData).map(([domain, skills]) => (
                <div key={domain}>
                  <div className="text-xs text-coffee-500 dark:text-cream-200/50 uppercase tracking-[0.12em] font-semibold mb-2.5">
                    {domain}
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
                    {(skills as readonly { id: string; label: string }[]).map((s) => (
                      <Link
                        ...
                        className="group flex items-center justify-between bg-cream-50 dark:bg-midnight-100 border border-coffee-700/10 dark:border-cream-200/8 rounded-2xl p-4 hover:scale-[1.01] hover:shadow-sm hover:border-accent/40 transition-all duration-200"
                      >
                        <span className="text-sm font-medium text-coffee-800 dark:text-cream-100">
                          {s.label}
                        </span>
                        <span className="text-coffee-500 dark:text-cream-200/40 group-hover:text-accent group-hover:translate-x-0.5 transition-transform">
                          →
                        </span>
                      </Link>
                    ))}
